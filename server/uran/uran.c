...
static void cleanup(void *_pArg)
{
	...
	//通知用户线程退出，放到哪里合适，开始？结束
	if (pThis->pUsrCallbck)
	{
		pThis->pUsrCallbck(0, NULL, 0);
	}	
...
static void *epoll_thread(void *_pArg)
{
	...
	//设置线程名字
	prctl(PR_SET_NAME, pManger->pucName);
...
* Return: -1/0
* *****************************************************************************/
INT32 UranMangerCreate(MANGER_TYPE _emRole, INT32 _iTrigNum, INT32 (*_pDeamonFunc)(void *), INT32 _pDeamonMSec, UINT8 *_pucName, void **_pThis)
{
	...
	pThis->pCleanup = NULL;
	pThis->pArg		= NULL;
	pThis->pUsrCallbck = NULL;
	if ((_pucName == NULL)||(strlen(_pucName) > MANGER_NAME_SIZE_MAX))
	{
		strcpy(pThis->pucName, "uran_default");
	}
	else
	{
		strcpy(pThis->pucName, _pucName);
	}
...
* Return: 
* *****************************************************************************/
INT32 UranMangerSetUsrCallbck(void *_pThis, void (*_pUsrCallbck)(INT32, void *, INT32))
{
	T_EvtMangerInfo *pThis = _pThis;
	pThis->pUsrCallbck	= _pUsrCallbck;
