...
void GKI_timer_update (int32_t ticks_since_last_update)
{
    ...
    /* Increment the number of ticks used for time stamps +1 */
    ...
    for (task_id = 0; task_id < GKI_MAX_TASKS; task_id++)
    {
        ...
        if (gki_cb.com.OSTaskTmr0[task_id] > 0)
        {
            ...
            if (gki_cb.com.OSTaskTmr0[task_id] <= 0)
            {
                ...
                gki_cb.com.OSTaskTmr0[task_id] = gki_cb.com.OSTaskTmr0R[task_id];   //OSTaskTmr0R重载值GKI_start_timer时候设定
...
**
*******************************************************************************/
uint8_t GKI_timer_queue_empty (void)
{
    uint8_t tt;
...
uint32_t GKI_get_remaining_ticks (TIMER_LIST_Q *p_timer_listq, TIMER_LIST_ENT  *p_target_tle)
{
    ...
    if (p_target_tle->in_use)
    {
        ...
        {
            TIM_LOGE("%s No timer entry in the list",__func__);
            return(0);
        }
    }
    else
    {
        TIM_LOGE("%s timer entry is not active",__func__);
    }
...
void GKI_remove_from_timer_list (TIMER_LIST_Q *p_timer_listq, TIMER_LIST_ENT  *p_tle)
{
    ...
    {
        TIM_LOGD("%s parameters error: p_tle = %x  ,  p_tle->in_use = %d , p_timer_listq->p_first = %x ",__func__, 
			p_tle, p_tle->in_use, p_timer_listq->p_first);
        return;
    }
